...
export function sanitizeInput(input) {
  if (typeof input === 'string') {
    // Only strip MongoDB operator characters ($), not dots or slashes
    // which are needed in emails, URLs, and file paths
  ...
  if (typeof input === 'object' && input !== null) {
    ...
    for (const [key, value] of Object.entries(input)) {
      // Strip $ from keys to prevent MongoDB operator injection
